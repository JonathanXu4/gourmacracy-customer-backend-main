...
router.get("/:id", (request, response, next) => {
  User.findById({ _id: request.params.id }, (error, result) => {
    if (error) {
...
router.post("/signin", function (req, response) {
  User.find({ email: req.body.email }).exec((error, userData) => {
    if (error) {
      ...
      response.send({
        text: "Error signing in. Try again later.",
        id: "Error",
        error: "Error",
      });
      ...
      bcrypt.compare(req.body.password, user.password, function (err, result) {
        if (result == true) {
          if (user._id == "65778900aa864e003654a729") {
          ...
          response.send({
            text:
              "Unable to login with this email address and password. " +
      ...
      response.send({
        text:
          "Unable to login with this email address and password. " +
          "Check your login information and try again later.",
        id: "error",
      });
    }
  });
